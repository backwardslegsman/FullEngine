...
Terrain runtime event and snapshot-diff export/import currently use
standard-library JSON Lines serialization and narrow schema parsing instead of
a third-party JSON dependency. Reconsider a maintained JSON library only if
future tooling needs broader parsing, schema migration, or richer nested
document generation.
