...
full_engine::TerrainStreamingTickEvent makeTick(const std::uint64_t sequence)
{
    ...
    event.scheduler.pendingLoadRequestCount = 33;
    event.scheduler.pendingJobCount = 34;
    event.scheduler.deferredWorkCount = 35;
    event.scheduler.peakDeferredWorkCount = 36;
    event.scheduler.runtimeBacklogCount = 37;
    ...
    event.scheduler.loadServiceRan = true;
    event.scheduler.externalCompletionsReconciled = true;
...
void testSingleTickExportsCounters(std::vector<std::string>& failures)
{
    ...
    expect(content.find("\"schedulerHasDecision\":true") != std::string::npos, "single tick exports scheduler presence", failures);
    expect(content.find("\"schedulerStatus\":\"Success\"") != std::string::npos, "single tick exports scheduler status", failures);
    expect(content.find("\"schedulerDecisionStatus\":\"RunStreamingAndAssetLoadJobs\"") != std::string::npos, "single tick exports scheduler decision", failures);
    ...
    expect(content.find("\"schedulerLoadServiceRan\":true") != std::string::npos, "single tick exports scheduler service phase", failures);
    expect(content.find("\"schedulerExternalCompletionsReconciled\":true") != std::string::npos, "single tick exports scheduler external completion phase", failures);
