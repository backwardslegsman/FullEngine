...
void testValidPayloadsPlanUploadWork(std::vector<std::string>& failures)
{
    ...
    expect(plan.records.size() == 3, "upload plan returns one record per payload", failures);
    expect(plan.summary.plannedCount == 3, "upload plan counts planned payloads", failures);
...
{
    full_engine::LoadedAssetPayload payload = meshPayload();
    payload.mesh.id = {};

    const full_engine::LoadedAssetUploadPlan plan =
        full_engine::buildLoadedAssetUploadPlan(&payload, 1);

    expect(plan.records.size() == 1, "invalid payload still produces diagnostic record", failures);
    ...
    expect(plan.summary.invalidPayloadCount == 1, "invalid payload summary is counted", failures);
}
...
void testOrderAndInactiveSlots(std::vector<std::string>& failures)
{
    ...
    const std::vector<full_engine::LoadedAssetPayload> payloads = {material, mesh, texture};
    const full_engine::LoadedAssetUploadPlan plan =
        full_engine::buildLoadedAssetUploadPlan(payloads.data(), payloads.size());

    expect(plan.summary.plannedCount == 3, "inactive slots do not affect upload planning", failures);
